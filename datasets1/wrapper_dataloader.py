...
class WrapperDataset(Dataset):
    def __init__(self, dataloader_a, dataloader_b, device, dataset_len_to_use=None):
        ...
        self.require_paired_data_a = False
        self.require_paired_data_b = False
        self.device = device
        ...
        if not self.require_paired_data_a and not self.require_paired_data_b:
            if self.dataset_a_larger:
                try:
                    ...
                    self.dataloader_b_iter = iter(self.dataloader_b)
                    dataset_b_data, dataset_b_label = next(self.dataloader_b_iter)
                dataset_a_data = next(self.dataloader_a_iter)
                for key in dataset_a_data.keys():
                    dataset_a_data[key] = dataset_a_data[key].to(self.device)
                dataset_a_label = 0
            else:
                try:
            ...
            return [dataset_a_data, dataset_a_label], \
