...
class BaseTrainer(object):
    ...
    def createCityscapesDataset(self, dataset_name, dataset_path, img_size, batch_size, semseg_num_classes,
        ...
        return train_loader_sensor, val_loader_sensor
    def createkittidataset(self, dataset_name, dataset_path, crop_size, batch_size):
            # databuilder = self.getDataloader(dataset_name)
            # train_dataset = databuilder(dataset_path, filenames_path='/home/thc/ess/ess-main/', 
            #      is_train=True, dataset='kitti', crop_size=(192, 344),
            #      scale_size=(664, 192))
            ...
            dataset_builder = self.getDataloader(dataset_name)
            fpath = '/home/thc/ess/ess-main/{}_list.txt'

            train_filenames = readlines(fpath.format("train"))
            val_filenames = readlines(fpath.format("test"))
            img_ext = '.png'
            train_dataset = dataset_builder(
                '/media/thc/Elements/kitti/', train_filenames, 192, 344,
                [0, -1, 1], 4, is_train=True, img_ext=img_ext)
            
            val_dataset = dataset_builder(
                '/media/thc/Elements/kitti/', val_filenames, 192, 344,
                [0, -1, 1], 4, is_train=False, img_ext=img_ext)
            dataset_loader = torch.utils.data.DataLoader
            train_loader_sensor = dataset_loader(train_dataset, batch_size=batch_size,
                                                num_workers=4,
                                                pin_memory=False, shuffle=True, drop_last=True)
            val_loader_sensor = dataset_loader(val_dataset, batch_size=1,
                                            num_workers=4,
                                            pin_memory=False, shuffle=False, drop_last=True)
            print('kitti num of batches: ', len(train_loader_sensor), len(val_loader_sensor))

            return train_loader_sensor, val_loader_sensor
    
    def createmvsecDataset(self, dataset_name, dataset_path, img_size, batch_size, nr_events_window, event_representation,
                      nr_temporal_bins):
        """
        Creates the validation and the training data based on the provided paths and parameters.
        """
        ...
        train_loader_sensor_b = torch.utils.data.DataLoader(dataset=train_set,
                                                batch_size=self.settings.batch_size_b,
                                                num_workers=4,
                                                shuffle=True,
                                                drop_last=True,
                                                pin_memory=True)  
        ...
        val_loader_sensor_b = torch.utils.data.DataLoader(dataset=val_set,
                                                ...
                                                drop_last=True,
                                                pin_memory=True)
        print('MVSEC num of batches: ', len(train_loader_sensor_b), len(val_loader_sensor_b))
    ...
    def combineDataloaders(self):
        ...
        self.train_loader = WrapperDataset(self.train_loader_sensor_a, self.train_loader_sensor_b, self.device, is_train=True)

    def createDataLoaders(self):
        if self.settings.dataset_name_a == 'kitti':
            out = self.createkittidataset(self.settings.dataset_name_a,
        ...
        out = self.createmvsecDataset(self.settings.dataset_name_b,
    ...
    def validationEpochs(self):
        ...
        with torch.no_grad():
            ...
            for key in self.result_metrics.keys():
                self.result_metrics[key] = self.result_metrics[key] / (self.batch + 1)

            result_lines = logging1.display_result(self.result_metrics)
           
            print(result_lines)
            self.validationEpoch(self.val_loader_sensor_b, 'sensor_b')
            for key in self.result_metrics.keys():
                self.result_metrics[key] = self.result_metrics[key] / (self.batch + 1)
            # save_path = os.path.join('result/a/', str(i_batch))
