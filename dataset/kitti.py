...
class kitti(BaseDataset):
    def __init__(self, data_path, filenames_path='./code/dataset/filenames/', 
        ...
        # txt_path = os.path.join(filenames_path)
        
        if is_train:
            filenames_path += '/train_list.txt'
        else:
            filenames_path += '/test_list.txt'        
    ...
    def __getitem__(self, idx):
        ...
        gt_path = '/media/thc/Elements/kittidepth/train/' + self.filenames_list[idx].split(' ')[1][-73:]
        filename = img_path.split('/')[-4] + '_' + img_path.split('/')[-1]

        image = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)  # [H x W x C] and C: BGR
        ...
        image = numpy.expand_dims(image, axis = 2)
        # depth = numpy.expand_dims(depth, axis = 2)
        if self.is_train:
